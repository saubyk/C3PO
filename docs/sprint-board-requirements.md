...
- **FR-W2.** Configuration is read from `.env` at startup. `WORKLOAD_TEAMS` is a comma-separated list of `org/team-slug` entries (e.g., `WORKLOAD_TEAMS=lightningnetwork/lnd-maintainers,lightningnetwork/lit-team`). An optional `WORKLOAD_ORGS` companion variable is a comma-separated list of bare org logins (e.g., `WORKLOAD_ORGS=other-org,vendor-org`); it is **search-scope only** and is used when the token can read an org's repos but not its team membership. It does not affect the roster.
- **FR-W3.** On startup, each `WORKLOAD_TEAMS` entry is resolved to its GitHub team members. The roster of monitored developers is the union of those memberships; the set of orgs to search is the union of the orgs implied by those entries and the orgs listed in `WORKLOAD_ORGS` (deduplicated). Resolution failures are logged and shown as a warning banner; the tab continues with the teams that did resolve.
